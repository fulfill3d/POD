...
namespace POD.Functions.Shopify.WebhookEndpoints
{
        ...
        IOptions<RequestTimeout> timeoutOptions)
    {
        ...
        public async Task<HttpResponseData> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = "webhooks/order/create")] HttpRequestData req,
            ILogger log)
        {
            // TODO Cancellation token is not working as expected. Write the messages to Cache to verify
            var cts = new CancellationTokenSource(_timeoutMilliseconds);
            var response = req.CreateResponse();
            try
            {
                log.LogInformation("Shopify Order Created function starting.");
                ...
                await orderService.OrderCreated(order, shop, cts.Token);
                log.LogInformation("Shopify Order Created function finished.");
